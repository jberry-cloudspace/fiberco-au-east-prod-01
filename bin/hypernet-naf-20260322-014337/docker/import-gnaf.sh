#!/bin/bash
# G-NAF Import Script for Hypernet
# Downloads, extracts, and imports the G-NAF dataset into PostgreSQL/PostGIS
set -euo pipefail

DB_HOST="${DB_HOST:-hypernet-naf-db}"
DB_USER="${DB_USER:-app}"
DB_NAME="${DB_NAME:-hypernet-naf}"
DB_PASS="${DB_PASS:-app}"
...
echo "=== G-NAF Import for Hypernet ==="
...
UNION ALL
SELECT 'Database size', pg_size_pretty(pg_database_size('hypernet-naf'));
VERIFYSQL
